...
exports.mail_valid = async (req, res, next) => {
  try {
    ...
    if (firstRegister == "A") {
      const { active } = await Student.findOne({
        cod_student: req.userId
      }).select("+active");
      console.log(active);
      if (active == true) {
        return next();
...
exports.user_login = async (req, res) => {
  try {
    ...
    if (firstRegister == "A") {
      ...
      const student = await Student.findOne({ cod_student: register }).select(
        "+password"
      );
      //if student registration not found
      if (!student) return res.status(400).send({ error: "user not found" });
    ...
    //if the first register return P, the user are student
      ...
      const teacher = await Teacher.findOne({ cod_Teacher: register }).select(
        "+password"
      );
      //if student registration not found
      if (!teacher) return res.status(400).send({ error: "user not found" });
      //if the password are not equal
...
exports.user_profile = async (req, res) => {
  try {
    ...
    if (isStudent) {
      ...
      res.send(isTeacher);
    } else res.send({ erro: "user not found" });
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    if (firstRegister == "A") {
      student = await Student.findOne(
        { cod_student: id },
        "name cod_student createdAt solutions"
      );
